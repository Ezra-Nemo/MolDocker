...
def is_in_ellipse(point, ellipse):
    ...
    # (x - c)^T * F * (x - c) = 1
    F = rot_mat.T.dot(np.diag(1 / np.array([a, b], dtype=np.float64) ** 2)).dot(rot_mat)
